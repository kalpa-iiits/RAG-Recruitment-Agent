...
def setup_sidebar():
    with st.sidebar:
        ...
        return {"openai_api_key": openai_api_key, "theme_color": theme_color}
...
def role_selection_section(role_requirements):
    ...
    with col1:
        role = st.selectbox(
            "Select the role you're applying for:", list(role_requirements.keys())
        )
    ...
    if not upload_jd:
        ...
        st.markdown(
            f"<p>Cutoff Score for selection: <b>{75}/100</b></p>",
            unsafe_allow_html=True,
        )
...
def display_analysis_results(analysis_result):
    ...
    st.markdown(
        ...
        'margin-bottom: 10px;">Powered by Euron Recruitment</div>',
        unsafe_allow_html=True,
    )
    ...
    with col2:
        if selected:
            st.markdown(
                "<h2 style='color:#4CAF50;'>✅ Congratulations! You have been shortlisted.</h2>",
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                "<h2 style='color:#d32f2f;'>❌ Unfortunately, you were not selected.</h2>",
                unsafe_allow_html=True,
            )
        ...
        for weakness in detailed_weaknesses:
            ...
            with st.expander(f"{skill_name} (Score: {score}/10)"):
                ...
                if "suggestions" in weakness and weakness["suggestions"]:
                    st.markdown(
                        "<strong>How to improve:</strong>", unsafe_allow_html=True
                    )
                ...
                if "example" in weakness and weakness["example"]:
                    st.markdown(
                        "<strong>Example addition:</strong>", unsafe_allow_html=True
                    )
                    st.markdown(
    ...
    with col2:
        report_content = f"""# Euron Recruitment - Resume Analysis Report
        ...
        href = (
            f'<a class="download-btn" href="data:text/plain;base64,{report_b64}" '
            f'download="euron_resume_analysis.txt">📊 Download Analysis Report</a>'
        )
...
def resume_improvement_section(has_resume, improve_resume_func=None):
    ...
    target_role = st.text_input(
        "Target role (optional):", placeholder="e.g., Senior Data Scientist at Google"
    )
        ...
        if improve_resume_func:
            ...
            for area, suggestions in improvements.items():
                with st.expander(f"Improvements for {area}", expanded=True):
                    st.markdown(
                        f"<p>{suggestions['description']}</p>", unsafe_allow_html=True
                    )
                    ...
                    if "before_after" in suggestions:
                        ...
                        st.markdown(
                            f"<pre>{suggestions['before_after']['before']}</pre>",
                            unsafe_allow_html=True,
                        )
                        st.markdown("</div>", unsafe_allow_html=True)
                        ...
                        st.markdown(
                            f"<pre>{suggestions['before_after']['after']}</pre>",
                            unsafe_allow_html=True,
                        )
                        st.markdown("</div>", unsafe_allow_html=True)
